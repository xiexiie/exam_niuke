                ...
                <ul>
                  <li v-for="(list, index1) in topic[1]" :key="index1">
                    <a href="javascript:;"
                      ...
                      <span :class="{'mark': topic[1][index1].isMark == true}"></span>{{index1+1}}
                    </a>
                  </li>
                </ul>
              </div>
                ...
                <ul>
                  <li v-for="(list, index2) in topic[2]" :key="index2">
                    <a href="javascript:;"
                      ...
                      <span :class="{'mark': topic[2][index2].isMark == true}"></span>{{topicCount[0]+index2+1}}</a>
                  </li>
                </ul>
              </div>
<!--              判断题部分-->
                ...
                <ul>
                  <li v-for="(list, index3) in topic[3]" :key="index3">
                    <a href="javascript:;"
                      ...
                      <span :class="{'mark': topic[3][index3].isMark == true}"></span>{{topicCount[0]+topicCount[1]+index3+1}}</a>
                  </li>
                </ul>
              </div>
<!--              简答题部分-->
              <div class="item">
                ...
                <ul>
                  <li v-for="(list, index4) in topic[4]" :key="index4">
                    <a href="javascript:;"
                       @click="short(index4)"
                       :class="{'border': index == index4 && currentType == 4,'bg': bg_flag && topic[4][index4] == true}">
                      <span :class="{'mark': topic[4][index4].isMark == true}"></span>{{topicCount[0]+topicCount[1]+topicCount[2]+index4+1}}</a>
                  </li>
                </ul>
              </div>
<!--              结束考试-->
...
<!--          考试内容-->
          <div class="content">
            ...
            <div class="change" v-if="currentType == 1">
            ...
            <div class="short" v-if="currentType == 4">
              ...
              <el-button type="success"  @click="save(topic[4][index],{number})">保存</el-button>
              <div class="analysis" v-if="isPractice">
                <ul>
                  <li>
                  ...
                  <li> <el-tag>题目解析：</el-tag><span class="right">{{topic[4][index].analysis == null ? '暂无解析': topic[4][index].analysis}}</span></li>
                </ul>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    //保存当前的简答题
    save(exam,quest){
      console.log(exam.questionId);
      console.log(quest.number);
      console.log(exam);
      console.log(quest);
      console.log(this.userInfo.name);
      console.log(this.userInfo.id);
      console.log(this.shortAnswer.answer);
      console.log(this.examData.source);
      console.log(this.examData.examCode);
      this.shortAnswer.source = this.examData.source
      this.shortAnswer.questionId = exam.questionId
      this.shortAnswer.questionMessage = exam.question
      this.shortAnswer.questionNumber = quest.number
      this.shortAnswer.studentId = this.userInfo.id
      this.shortAnswer.studentName = this.userInfo.name
      this.shortAnswer.examCode = this.examData.examCode
      this.$axios({
        url: '/api/shortAnswer',
        method: 'post',
        data: {
    ...
    commit() {
      ...
      //   let right=null
      //
      //   if(right == this.topic[4][index].answer){
      //     finalScore += this.topic[4][index].score
      //   }
      // })
